 ...
 * the note range and sound playback.
 */
...
function loadSettings(): SightReadingSettings {
  ...
  return { fromNote: DEFAULT_FROM, toNote: DEFAULT_TO, playSound: true, selectedDeviceId: undefined };
}
...
export default function SightReading() {
  ...
  // ---- Derived ----
  /** The actual note pool (at actual pitch, not display pitch). */
  ...
  useEffect(() => {
    saveSettings({ fromNote, toNote, playSound, selectedDeviceId });
  }, [fromNote, toNote, playSound, selectedDeviceId]);
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }} size={24}>
          ...
          <div>
            ...
            >
              {t('sightReading.noteRangeHint', { count: notePool.length })}
            </Text>
          </div>
